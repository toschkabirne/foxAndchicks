...
fn total_height() -> i32 {
    if graph_enabled() {
        settings::SCREEN_HEIGHT + GRAPH_HEIGHT
    } else {
...
pub fn draw_population_graph_fullscreen(history: &[(usize, usize)], current_frame: usize) {
    // Reuse your existing graph drawer by temporarily treating the graph as the whole window.
    // This is basically a copy of draw_population_graph, but with (0,0) and SCREEN_HEIGHT.
    let area_x = 0.0;
    let area_y = 0.0;
    let area_w = (settings::SCREEN_WIDTH + STATS_PANEL_WIDTH) as f32;
    let area_h = settings::SCREEN_HEIGHT as f32;

    draw_rectangle(area_x, area_y, area_w, area_h, Color::from_rgba(22, 22, 22, 255));
    draw_text("Population over time (press G to return)", area_x + 45.0, area_y + 24.0, 22.0, WHITE);

    if history.is_empty() {
        draw_text("No data yet", area_x + 45.0, area_y + 60.0, 22.0, WHITE);
        return;
    }

    // padding (slightly larger since we have more space)
    let pad_l = 60.0;
    let pad_r = 25.0;
    let pad_t = 40.0;
    let pad_b = 40.0;
    ...
    let plot_w = area_w - pad_l - pad_r;
    let plot_h = area_h - pad_t - pad_b;
    ...
    // limit samples ~ one per pixel
    let max_samples = plot_w.max(1.0) as usize;
    let n = history.len();
    ...
    for frac in [0.0_f32, 0.5, 1.0] {
        ...
        draw_text(&format!("{}", label_val), area_x + 12.0, y + 6.0, 18.0, Color::from_rgba(200, 200, 200, 255));
    }

    let len = slice.len().max(2);
    let dx = plot_w / (len as f32 - 1.0);

    let mut prev_pred: Option<Vec2> = None;
    let mut prev_prey: Option<Vec2> = None;

    for (i, &(pred, prey)) in slice.iter().enumerate() {
        let x = plot_x0 + i as f32 * dx;
    ...
    if current_frame >= start && current_frame < history.len() {
        let local = current_frame - start;
        let x = plot_x0 + (local as f32) * dx;
        draw_line(x, plot_y0, x, plot_y0 + plot_h, 1.0, Color::from_rgba(180, 180, 180, 180));
    }

    // legend
    let legend_y = area_y + area_h - 14.0;
...
/// Draws a simple predator/prey population graph (Lotka–Volterra style).
/// `history`: (pred_count, prey_count) per frame.
pub fn draw_population_graph(history: &[(usize, usize)], current_frame: usize) {
    if !graph_enabled() {
        return;
    }

    // Graph area is below the simulation.
    let area_x = 0.0;
    let area_y = settings::SCREEN_HEIGHT as f32;
    let area_w = (settings::SCREEN_WIDTH + STATS_PANEL_WIDTH) as f32;
    let area_h = GRAPH_HEIGHT as f32;

    // Always draw the panel background so you can see it's enabled
    draw_rectangle(area_x, area_y, area_w, area_h, Color::from_rgba(22, 22, 22, 255));
    draw_line(
        area_x,
        area_y,
    ...
    draw_text("Population over time", area_x + 45.0, area_y + 16.0, 18.0, WHITE);

    if history.is_empty() {
        draw_text("No data yet", area_x + 45.0, area_y + 40.0, 18.0, WHITE);
        return;
    }
    ...
    // show last ~one point per pixel for performance/readability
    let max_samples = plot_w.max(1.0) as usize;
    let n = history.len();
    let start = if n > max_samples { n - max_samples } else { 0 };
    let slice = &history[start..];
    ...
    for (i, &(pred, prey)) in slice.iter().enumerate() {
        ...
        if let Some(p) = prev_pred {
            draw_line(p.x, p.y, pred_pt.x, pred_pt.y, 2.0, settings::PRED_COLOR);
        }
        if let Some(p) = prev_prey {
            draw_line(p.x, p.y, prey_pt.x, prey_pt.y, 2.0, settings::PREY_COLOR);
        }
    ...
    // current frame marker (if visible)
    if current_frame >= start && current_frame < history.len() {
        let local = current_frame - start;
        let x = plot_x0 + (local as f32) * dx;
        draw_line(
    ...
    draw_text("Population over time", plot_x0, area_y + 16.0, 18.0, WHITE);
    let legend_y = area_y + area_h - 6.0;
    draw_text("Predators", plot_x0, legend_y, 16.0, settings::PRED_COLOR);
    draw_text("Preys", plot_x0 + 120.0, legend_y, 16.0, settings::PREY_COLOR);
