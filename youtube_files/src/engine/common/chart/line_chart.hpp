...
struct LineChart : public sf::Drawable, public sf::Transformable
{
    // Data
    ...
    mutable sf::VertexArray va_area;
    mutable sf::VertexArray va_line;
    ...
    static Vec2 getIntersection(Vec2 p1, Vec2 n1, Vec2 p2, Vec2 n2)
    {
        ...
        float const m1 = (p1End.y - p1.y) / (p1End.x - p1.x); // slope of line p1->n1
        float const m2 = (p2End.y - p2.y) / (p2End.x - p2.x); // slope of line p2->n2

        float const b1 = p1.y - m1 * p1.x; // y-intercept of line p1->n1
        float const b2 = p2.y - m2 * p2.x; // y-intercept of line p2->n2

        float const px = (b2 - b1) / (m1 - m2); // collision x
        float const py = m1 * px + b1; // collision y
    ...
    void updateGeometry() const
    {
        ...
        values.foreach([&](uint32_t i, float v) {
            float const normalized_x = to<float>(i) * dx;
