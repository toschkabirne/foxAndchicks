...
public:
    ...
    // Adds a mouse pressed callback
    void linkMouseButtonStateToBool(sf::Mouse::Button button, bool& state)
    {
        m_event_map.addMousePressedCallback(button, [&state](sfev::CstEv) { state = true; });
        m_event_map.addMouseReleasedCallback(button, [&state](sfev::CstEv) { state = false; });
    }
