...
public:
    ...
    void registerDefaultCallbacks(bool use_viewport_callbacks)
    {
        ...
        m_event_manager.addEventCallback(sf::Event::Resized, [&](sfev::CstEv ev) {
            Vec2 const new_size_f{static_cast<float>(ev.size.width), static_cast<float>(ev.size.height)};
            m_render_context->m_viewport_handler.state.window_size = new_size_f;
        });
