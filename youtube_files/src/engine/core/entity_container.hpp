...
struct EntityContainer
{
    ...
    static siv::Vector<TEntity> data;
    ...
    static bool isValid(const Handle& ref)
    {
        return ref.id.class_id == class_id && data.isValid(ref.id.instance_id, ref.validity);
    }

    static uint64_t getValidity(pez::core::ID id)
    {
        return data.getValidityID(id);
    }
};
...
template<typename T>
siv::Vector<T> EntityContainer<T>::data = {};
