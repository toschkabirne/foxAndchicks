...
struct EntityManager
{
    ...
    using CheckValidityCallback = bool(*)(const Handle&);
    using GetValidityCallback = uint64_t(*)(pez::core::ID);
    ...
    std::vector<CheckValidityCallback> check_validity_callbacks;
    std::vector<GetValidityCallback>   get_validity_callbacks;
    ...
    void registerEntity()
    {
        ...
        check_validity_callbacks.push_back(EntityContainer<T>::isValid);
        get_validity_callbacks.push_back(EntityContainer<T>::getValidity);
    }
    ...
    bool isValid(const Handle& ref)
    {
        return check_validity_callbacks[EntityContainer<TEntity>::class_id](ref);
    }
};
